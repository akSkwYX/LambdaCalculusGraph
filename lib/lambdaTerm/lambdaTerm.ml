...
module NoParenthesis : LambdaTerm = struct
  include Base

  let compose_lambda t (l : Token.t list) =
    match l with
    | {lexeme=N n; _} :: rest -> App (List.init n (Fun.const t)), rest
    | _ -> t, l

  let extend (t : Token.t) =
    match t.lexeme with
    | Number n ->
      compose_lambda (Var {lexeme=Var "f"; index=(-1)}) [{lexeme= N n; index=(-1)}] |> fst
    | Succ -> 
